...
# Function to handle group-object collisions
...
# Function to handle group-group collisions
...
def click(pos):
    global started, score, lives, max_rocks
...
def draw(canvas):
    global time, started, lives, score, max_rocks, rock_group
    ...
    process_spriteGrp(rock_group, canvas)
    process_spriteGrp(explosion_group, canvas)
    process_spriteGrp(missile_group, canvas)
...
# fuction to draw and update the sprites
def process_spriteGrp(group, canvas):
    remove_set = set([])
    for sprite in group:
        sprite.update()
        sprite.draw(canvas)
